...
| 组件 | 当前版本 | 部署状态 | 备注 |
|------|----------|----------|------|
| 主站 Portal | **v2.3.5** | 代码已推 `main`；前端 Pages 自动生效（约 1–2 分钟）；SCF（`report-portal-upload`）未改动、无需重传 | **紧急回归修复**：v2.3.4 重写侧边栏时漏写 `cat.children.forEach(...)` 循环包裹，导致悬空 `});` 整段 JS 语法崩溃、侧边栏全部分类不显示；v2.3.5 补回循环，保留 v2.3.4 功能意图（移除「直属文件」分组 + 父分类名可点击查看其下全部文件） |
...
|------|------|------|------|------|--------|------|
| **v2.3.5** | 2026-09-22 | 主站 Portal | PATCH | **紧急回归修复**：v2.3.4 移除「直属文件」分组时重写侧边栏 `nav-item`/`nav-children` 渲染块，误删 `cat.children.forEach(function(child) {` 循环开头（仅保留 `var count=...` 与闭合 `});`），导致树遍历回调中出现悬空 `});`/悬空 `else`、整段 JS `SyntaxError`、侧边栏分类渲染函数崩溃、**全部文件夹不显示**；v2.3.5 补回 `cat.children.forEach(function(child) {` 循环包裹，功能意图不变（移除「直属文件」虚拟分组 + 父分类名可点击进入其下全部文件视图） | 前端改动，无 SCF 部署包（推 GitHub Pages 即生效） | 用户反馈"现在全部的文件夹都看不到了"触发；用 `new Function(code)` 语法检查确凿定位 `Unexpected token ')'`；属主站改动（用户授权） |
