...
# 1) Ensure scf_bootstrap is LF (defensive; .gitattributes already enforces)
...
# 2) Archive v2.3.0 (do not overwrite existing; only create)
os.makedirs(VERDIR, exist_ok=True)
for fn in ["index.html", "server.py", "scf_bootstrap", "test_v230_dynamic.py"]:
...
scratch = ["_frontend_check.js", "_dbg.py", "_dbg2.py", "_dbg_srv.py", "_dbg_srv2.py",
...
with zipfile.ZipFile(zipname, "w", zipfile.ZIP_DEFLATED) as z:
    z.write(os.path.join(ROOT, "server.py"), "server.py", compress_type=zipfile.ZIP_DEFLATED)
...
with zipfile.ZipFile(zipname) as z:
    ...
    print("  scf_bootstrap CRLF count:", sb_bytes.count(b'\r\n'), "| LF-only:", sb_bytes.count(b'\n')-sb_bytes.count(b'\r\n'))
