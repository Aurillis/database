...
· KPI（4 个，顺序固定）：机会评分 / 风险评分 / 样本月销量(件) / 加权成交价

· 表1「核心指标」表头：核心指标 | 数值 | 说明
  行（固定）：候选→核心产品(ASIN数) / 样本月销量·销售额 / 加权成交价 / Top 类型 / Top5 品牌销售额占比 / 新品均销额 / Amazon 自营

· 表2「十维机会评分」表头：十维机会评分 | 分数 | 要点
  行（固定十个维度，顺序不可变）：市场容量 / 增长想象空间 / 竞争集中度 / 价格承接 / 评分壁垒 / 产品差异化 / 进入门槛 / 内容教育价值 / 复购组合 / 综合机会

· 表3「品牌榜」表头：品牌销售额 Top | ASIN数 | 销量 | 销售额 | 均价

· callouts（固定三条线索）：①场景矩阵（按使用场景拆分 ASIN数/销量/销售额）②核心产品类型与进入机会 ③数据诚实声明（区分卖家精灵实测字段 vs 语义分类字段）"""
...
def demo_generic(topic):
    ...
    summary = (
        f"「{topic}」的自动调研报告（<b>demo 模式·示意数据</b>）。当前未接入在线大模型，"
...
def demo_amz_us():
    ...
    return {
        "note": "【固定模板】本维度锁定遵循「US Bladder Control Devices 盆底肌训练器机会评分看板」标准结构：KPI(机会评分/风险评分/样本月销量/加权成交价) → 核心指标 → 十维机会评分 → 品牌销售额 Top → 场景矩阵/产品类型/数据诚实。结构不随主题漂移。数据来源：卖家精灵 MCP（SellerSprite）抓取结果；样本月销量/销售额/价格/评分/评论/BSR 来自卖家精灵；产品类型与功能原理为基于标题的语义分类。",
        "kpis": [
            {"v": "67", "l": "机会评分"},
            {"v": "58", "l": "风险评分"},
            {"v": "10,076", "l": "样本月销量(件)"},
        ...
        "tables": [
            {"head": ["核心指标", "数值", "说明"], "rows": [
                ...
                ["加权成交价", "$77.48", "销售额 / 销量"],
                ["Top 类型", "EMS 电刺激训练器", "销量最高产品路线"],
                ["Top5 品牌销售额占比", "73.5%（销量 60.6%）", "Intimate Rose 居首"],
            ...
            {"head": ["品牌销售额 Top", "ASIN数", "销量", "销售额", "均价"], "rows": [
        ...
        "callouts": [
            "场景矩阵：产后/盆底肌力量恢复 19 ASIN·5,978 件·$551,476；轻中度漏尿/膀胱控制 13 ASIN·2,883 件·$428,022——后者对专业性/安全性/隐私更敏感，是差异化入口。",
            "核心产品类型：EMS 电刺激训练器为销量 Top 类型；App/生物反馈均价最高（Perifit $149）。进入机会在可解释训练反馈、探头安全、程序舒适度与合规表达。",
            "数据诚实：销量/销售额/价格/评分/评论/BSR 来自卖家精灵；类型与功能为语义分类，非厂商披露。",
        ],
        "summary": "美国盆底肌训练器已是明确亚马逊竞品池（月销万级），头部占位但中高端电子训练器/组合套装仍有进入窗口。",
    }
...
def build_report(topic, dims, source="template", llm=None):
    ...
    if source == "llm" and llm and llm.get("key"):
        ...
        summary_parts = [f"{DIMS[k]['title']}：{sections_out[k].get('summary','')}" for k in sections_out]
        summary = f"「{topic}」自动调研（大模型实时生成）。" + "；".join(summary_parts)
        top_kpis = []
...
def main_handler(event, context):
    ...
    try:
        ...
        captured["body"] = json.dumps({"error": f"internal: {e}"}, ensure_ascii=False)
    return {
...
if __name__ == "__main__":
    ...
    with socketserver.TCPServer(("0.0.0.0", PORT), Handler) as httpd:
        print(f"调研台 ResearchDeck (A2) 已启动 → http://localhost:{PORT}  [mode={MODE}]")
        if MODE == "demo":
