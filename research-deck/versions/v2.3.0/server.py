...
# ===========================================================================
# v2.3.0：动态 Research Creation Model（Dynamic Scope / Research Recipe）
# ---------------------------------------------------------------------------
# 取代「选择固定研究模板 → 按固定目录生成报告」的核心逻辑。
# 原则：Research Question 决定 Research Scope，而不是固定模板决定 Research 内容。
# FDA / 法规 / 专利 / 供应链 等不再强制进入每次研究，而是动态 Research Capability。
# 旧 RESEARCH_TYPES / DIMS / call_llm_dim 保留（历史兼容 + 标准/专题/综合立项仍可用）。
# ===========================================================================

# —— 动态 Research Scope 能力目录（可扩展；不是固定报告章节）——
# 每项：title 展示名 / icon / desc / methods 推荐研究方法 / sources 推荐来源 /
#      trigger 关键词（离线 heuristic 用）/ medical 是否医疗相关（条件型判定用）
RESEARCH_SCOPES = {
    "market":      {"title": "市场", "icon": "📊", "desc": "规模 / 集中度 / 机会矩阵 / 进入时机",
                    "methods": ["TAM/SAM/SOM", "Top-down", "Bottom-up", "CAGR 分析", "市场集中度(CR5/CR10)", "增长驱动分析"],
                    "sources": ["行业报告", "Statista", "官方统计", "卖家精灵/电商数据", "券商研报"], "trigger": ["市场", "规模", "机会", "赛道", "增长"]},
    "user":        {"title": "用户", "icon": "👥", "desc": "痛点 / 场景 / 未满足需求（带信号强度）",
                    "methods": ["VOC", "Review Mining", "JTBD", "User Journey", "痛点聚类", "未满足需求分析"],
                    "sources": ["Amazon 评论", "Reddit", "Facebook", "Quora", "独立评测", "品牌官网"], "trigger": ["用户", "差评", "痛点", "为什么", "退货", "需求"]},
    "product":     {"title": "产品", "icon": "🔬", "desc": "型号拆解 / 参数盘点 / 硬件方案横向对比",
                    "methods": ["型号拆解", "参数盘点", "硬件方案对比", "功能矩阵"],
                    "sources": ["官方产品页", "产品手册", "拆解评测", "电商详情页"], "trigger": ["型号", "参数", "拆解", "产品对比"]},
    "comp":        {"title": "竞品", "icon": "⚔️", "desc": "定位 / 卡位矩阵 / 空位",
                    "methods": ["竞品格局", "Feature Matrix", "Benchmark", "定位地图", "产品拆解", "空白点分析"],
                    "sources": ["竞品官网", "电商页面", "评测", "专利", "财报"], "trigger": ["竞品", "对手", "对标", "卡位", "差异化"]},
    "tech":        {"title": "技术", "icon": "⚙️", "desc": "核心技术与工程方案 / 路线对比 / 实现难度",
                    "methods": ["技术地图", "路线对比", "参数 Benchmark", "架构分析", "可行性分析", "技术拆解"],
                    "sources": ["论文", "专利", "技术文档", "厂商资料", "标准"], "trigger": ["技术", "参数", "方案", "实现", "电路", "算法", "频率", "脉宽"]},
    "clinical":    {"title": "临床证据", "icon": "🩺", "desc": "临床文献 / 指南 / 证据等级",
                    "methods": ["临床证据综述", "指南比对", "RCT/Meta 分析", "证据等级评估"],
                    "sources": ["PubMed", "ClinicalTrials.gov", "临床指南", "综述文献", "厂商临床报告"], "trigger": ["临床", "证据", "文献", "指南", "适应症", "疗效"], "medical": True},
    "science":     {"title": "科学证据", "icon": "🔬", "desc": "机理 / 基础科学研究",
                    "methods": ["机理综述", "基础文献", "机制分析"],
                    "sources": ["Science/Nature", "基础医学文献", "综述"], "trigger": ["机理", "科学", "原理", "基础"]},
    "reg":         {"title": "法规", "icon": "🛡️", "desc": "监管路径判定 / 合规边界（高层动态能力，非仅 FDA）",
                    "methods": ["监管路径判定", "合规边界分析", "标签/宣称审查", "标准映射"],
                    "sources": ["监管机构官网(FDA/EMA/NMPA)", "21 CFR", "MDR", "标准文档", "法规数据库"], "trigger": ["法规", "合规", "监管", "宣称", "标签", "资质"], "medical": True},
    "fda":         {"title": "FDA", "icon": "🇺🇸", "desc": "Device Classification / Product Code / 510(k) / Predicate / Recognized Standards",
                    "methods": ["Device Classification 判定", "Product Code 查询", "510(k) 路径", "Predicate 检索", "标准识别", "标签/宣称审查"],
                    "sources": ["FDA 数据库", "510(k) Summary", "Product Code 数据库", "Recognized Standards", "标签法规"], "trigger": ["FDA", "510(k)", "predicate", "产品分类", "上市路径"], "medical": True, "parent": "reg"},
    "patent":      {"title": "专利", "icon": "📜", "desc": "专利布局 / FTO / 侵权风险 / 技术壁垒",
                    "methods": ["专利地图", "受让人分析", "技术聚类", "权利要求分析", "FTO 筛查"],
                    "sources": ["Google Patents", "USPTO", "CNIPA", "EPO", "厂商专利"], "trigger": ["专利", "patent", "FTO", "侵权", "壁垒", "创新空间"]},
    "supply":      {"title": "供应链", "icon": "📦", "desc": "工厂筛选 / 供应商 / 交期 / 产能 / 品质体系",
                    "methods": ["工厂筛选", "供应商评估", "产能/交期评估", "品质体系审核"],
                    "sources": ["1688", "展会", "行业名录", "验厂报告"], "trigger": ["工厂", "供应", "量产", "产能", "交期", "供应商", "制造"]},
    "bom":         {"title": "BOM/成本", "icon": "🧾", "desc": "BOM 拆解 / 成本结构 / 降本",
                    "methods": ["BOM 拆解", "成本结构分析", "降本路径"],
                    "sources": ["BOM 表", "元器件报价", "工厂报价"], "trigger": ["BOM", "成本", "降本", "物料"]},
    "material":    {"title": "材料", "icon": "🧱", "desc": "材料选型 / 生物相容 / 可持续",
                    "methods": ["材料选型", "生物相容性评估", "材料对比"],
                    "sources": ["材料数据库", "文献", "供应商资料"], "trigger": ["材料", "生物相容", "材质"]},
    "safety":      {"title": "安全", "icon": "⚠️", "desc": "安全风险 / 电气安全 / 使用安全",
                    "methods": ["安全风险分析", "电气安全评估", "使用安全审查"],
                    "sources": ["安全标准", "不良事件数据库", "文献"], "trigger": ["安全", "风险", "电气", "不良事件"]},
    "risk":        {"title": "风险", "icon": "🚧", "desc": "项目 / 产品风险矩阵",
                    "methods": ["风险矩阵", "FMEA", "缓解策略"],
                    "sources": ["文献", "竞品反馈", "法规"], "trigger": ["风险", "风险矩阵"]},
    "channel":     {"title": "渠道", "icon": "🛒", "desc": "Amazon / DTC / Retail / 医疗渠道",
                    "methods": ["渠道结构分析", "进场条件评估"],
                    "sources": ["平台规则", "行业报告"], "trigger": ["渠道", "Amazon", "DTC", "零售", "医院", "诊所"]},
    "pricing":     {"title": "定价", "icon": "💲", "desc": "定价区间 / 价格带 / 价值锚",
                    "methods": ["定价带分析", "价值锚定", "竞品定价对比"],
                    "sources": ["电商页面", "竞品报价"], "trigger": ["定价", "价格", "售价"]},
    "bizmodel":    {"title": "商业模式", "icon": "💡", "desc": "商业模式 / 变现路径",
                    "methods": ["商业模式画布", "变现路径分析"],
                    "sources": ["财报", "行业分析"], "trigger": ["商业模式", "变现", "盈利"]},
    "brand":       {"title": "品牌", "icon": "🏷️", "desc": "品牌定位 / 叙事",
                    "methods": ["品牌定位", "叙事分析"],
                    "sources": ["品牌官网", "社媒"], "trigger": ["品牌", "定位", "叙事"]},
    "trend":       {"title": "趋势", "icon": "📈", "desc": "行业 / 消费趋势",
                    "methods": ["趋势扫描", "信号分析"],
                    "sources": ["趋势报告", "社媒", "展会"], "trigger": ["趋势", "趋势"]},
    "industry":    {"title": "行业", "icon": "🏭", "desc": "产业链 / 行业结构",
                    "methods": ["产业链分析", "行业结构"],
                    "sources": ["行业报告", "年报"], "trigger": ["行业", "产业链"]},
    "policy":      {"title": "政策", "icon": "📋", "desc": "政策 / 补贴 / 贸易",
                    "methods": ["政策梳理", "影响评估"],
                    "sources": ["政府官网", "政策数据库"], "trigger": ["政策", "补贴", "贸易"]},
    "service":     {"title": "售后", "icon": "🔧", "desc": "售后 / 客服 / 退换",
                    "methods": ["售后痛点分析", "服务设计"],
                    "sources": ["评论", "客服记录"], "trigger": ["售后", "客服", "退换"]},
    "mfg":         {"title": "制造", "icon": "🏭", "desc": "制造工艺 / 良率 / 资质",
                    "methods": ["工艺分析", "良率评估", "生产资质审查"],
                    "sources": ["工厂资料", "工艺文档"], "trigger": ["制造", "工艺", "良率", "生产"]},
    "sustain":     {"title": "可持续性", "icon": "🌱", "desc": "环保 / ESG",
                    "methods": ["可持续性评估", "合规审查"],
                    "sources": ["法规", "标准"], "trigger": ["可持续", "环保", "ESG"]},
    "other":       {"title": "其他", "icon": "➕", "desc": "自定义范围",
                    "methods": [], "sources": [], "trigger": []},
}

# —— Research Recipe（研究策略模板，非固定报告目录）——
# Recipe 表达「面对某类任务，通常该考虑哪些 Scope/Method/Source/Output」+ 条件判断。
# 最终生成「建议 Research Plan」，不强制目录。
RESEARCH_RECIPES = {
    "new_opportunity": {
        "title": "新产品机会研究",
        "default_scopes": ["market", "user", "comp", "product", "tech"],
        "conditions": [
            {"if": "medical_or_health_claim", "add": ["clinical", "reg"]},
            {"if": "innovation_tech", "add": ["patent"]},
            {"if": "go_to_mass_production", "add": ["supply", "bom", "mfg"]},
            {"if": "not_ecom_product", "remove": ["channel"]},
        ],
    },
    "parameter_definition": {
        "title": "参数/技术方案定义",
        "default_scopes": ["tech", "clinical", "comp"],
        "conditions": [
            {"if": "medical_market_us", "add": ["fda", "reg"]},
            {"if": "innovation_tech", "add": ["patent"]},
        ],
    },
    "user_pain": {
        "title": "用户痛点 / 差评研究",
        "default_scopes": ["user", "comp"],
        "conditions": [
            {"if": "medical_or_health_claim", "add": ["clinical"]},
        ],
    },
    "supply_massprod": {
        "title": "供应链 / 量产研究",
        "default_scopes": ["tech", "bom", "supply", "mfg", "safety"],
        "conditions": [
            {"if": "medical_product", "add": ["reg", "material"]},
        ],
    },
    "patent_landscape": {
        "title": "专利布局研究",
        "default_scopes": ["patent", "tech", "comp"],
        "conditions": [],
    },
    "free": {
        "title": "自由研究（完全交给 AI 判断）",
        "default_scopes": [],
        "conditions": [],
    },
}

# —— 离线 heuristic：根据 Question + Context 推断条件标志 ——
def _scope_conditions(question, context):
    q = (question or "").lower()
    ctx = context or {}
    markets = [str(m).lower() for m in (ctx.get("markets") or [])]
    ptype = str(ctx.get("product_type") or ctx.get("product_profile") or "").lower()
    pclaim = str(ctx.get("product_claims") or ctx.get("background") or "").lower()
    blob = q + " " + ptype + " " + pclaim
    med = any(k in blob for k in ["医疗", "治疗", "therapy", "medical", "器械", "fda", "临床", "clinical", "health"]) or ctx.get("medical")
    us = ("us" in markets) or ("美国" in markets) or ("united states" in markets) or ("usa" in markets)
    eu = ("eu" in markets) or ("欧洲" in markets) or ("europe" in markets)
    innovation = any(k in q for k in ["专利", "patent", "fto", "创新", "innovation", "技术路线", "壁垒"])
    massprod = any(k in q for k in ["工厂", "量产", "产能", "制造", "bom", "成本", "供应", "交期", "生产"])
    ecom = any(k in blob for k in ["amazon", "电商", "亚马逊", "零售", "dtc"])
    flags = {
        "medical_or_health_claim": med,
        "medical_market_us": med and us,
        "medical_product": med,
        "innovation_tech": innovation,
        "go_to_mass_production": massprod,
        "not_ecom_product": not ecom,
    }
    if eu and med:
        flags["medical_market_eu"] = True
    return flags

# —— 离线 heuristic 推荐 Scope（Level: core/optional/skip）——
def _suggest_scope_heuristic(question, context):
    flags = _scope_conditions(question, context)
    q = (question or "")
    ql = q.lower()
    core = ["market", "user", "comp", "product", "tech"]
    optional, skip = [], []
    for sid, sc in RESEARCH_SCOPES.items():
        if sid in ("market", "user", "comp", "product", "tech", "other"):
            continue
        hits = [t for t in sc.get("trigger", []) if t and t.lower() in ql]
        if hits:
            if sid not in core:
                core.append(sid)
    if flags.get("medical_or_health_claim"):
        if "clinical" not in core: core.append("clinical")
        if "reg" not in core: core.append("reg")
        if flags.get("medical_market_us") and "fda" not in core: core.append("fda")
        if flags.get("medical_market_eu") and "reg" not in core: core.append("reg")
    if flags.get("innovation_tech") and "patent" not in core:
        optional.append("patent")
    if flags.get("go_to_mass_production"):
        for s in ("supply", "bom", "mfg", "safety"):
            if s not in core: core.append(s)
    if not flags.get("medical_or_health_claim"):
        for s in ("fda", "reg"):
            if s not in core and s not in optional:
                skip.append(s)
    if not flags.get("innovation_tech") and "patent" not in core:
        skip.append("patent")
    if not flags.get("go_to_mass_production") and "supply" not in core:
        skip.append("supply")
    scopes = []
    for sid in core:
        sc = RESEARCH_SCOPES.get(sid)
        if not sc: continue
        scopes.append({"name": sc["title"], "id": sid, "level": "core", "reason": _scope_reason(sid, flags, q)})
    for sid in optional:
        sc = RESEARCH_SCOPES.get(sid)
        if not sc: continue
        scopes.append({"name": sc["title"], "id": sid, "level": "optional", "reason": _scope_reason(sid, flags, q)})
    for sid in skip:
        sc = RESEARCH_SCOPES.get(sid)
        if not sc: continue
        scopes.append({"name": sc["title"], "id": sid, "level": "skip", "reason": _scope_reason(sid, flags, q)})
    return scopes

def _scope_reason(sid, flags, q):
    m = {
        "clinical": "刺激频率/脉宽/Work-Rest 等参数需临床文献或指南支持，否则无法判断合理性。",
        "fda": "美国市场医疗/健康宣称产品需判定 Device Classification / 510(k) / Predicate / 标签宣称边界。",
        "reg": "医疗或健康宣称产品在目标市场需明确监管路径与合规边界。",
        "patent": "涉及技术创新/结构创新/竞品壁垒时，应评估 FTO 与专利布局空间。",
        "supply": "涉及工厂筛选/量产/成本/BOM 时应研究供应链与制造可行性。",
        "bom": "落地量产需拆解 BOM 与成本结构。",
        "mfg": "量产工艺/良率/生产资质影响可制造性。",
        "safety": "涉及电气/使用安全需评估风险边界。",
        "tech": "问题核心是技术方案/参数定义，技术是主研究范围。",
        "comp": "需对比主流产品参数与方案。",
        "user": "需确认真实用户痛点与场景。",
        "market": "需判断市场机会与进入时机。",
        "product": "需拆解产品型号与参数。",
    }
    return m.get(sid, "根据研究问题，该范围可能被涉及，建议作为可选。")

# —— LLM 推荐 Scope（在线）——
def _suggest_scope_llm(question, context, llm):
    flags = _scope_conditions(question, context)
    ctx_lines = []
    for k in ("markets", "product_stage", "channels", "background", "product_type", "product_claims"):
        v = (context or {}).get(k)
        if v:
            ctx_lines.append("{}: {}".format(k, v if isinstance(v, str) else json.dumps(v, ensure_ascii=False)))
    ctx_txt = "\n".join(ctx_lines) or "（无额外背景）"
    scope_catalog = "\n".join("- {} (id={}): {}".format(s["title"], sid, s["desc"]) for sid, s in RESEARCH_SCOPES.items())
    prompt = (
        "你是产品研究范围的判断专家。根据用户提出的「研究问题」与「背景」，从下面的 Research Scope 能力目录中，"
        "挑选本次研究真正需要的范围（不要全选，只选相关项），并为每个范围标注推荐程度与推荐原因。\n\n"
        "【研究问题】\n{question}\n\n"
        "【背景 Context】\n{ctx}\n\n"
        "【可选 Research Scope 能力目录（动态能力，非固定章节）】\n{catalog}\n\n"
        "【条件提示（仅供判断参考）】\n{flags}\n\n"
        "要求输出 JSON：\n"
        "{{\n"
        '  "scopes": [{{"id":"<scope_id>","level":"core|optional|skip","reason":"<中文推荐原因，1-2句>"}}],\n'
        '  "methods": [{{"scope":"<scope_id>","name":"<研究方法>"}}],\n'
        '  "sources": [{{"name":"<来源名>","priority":"high|med|low"}}],\n'
        '  "expected_outputs": ["<期望输出的第1项>", "..."],\n'
        '  "research_plan": {{\n'
        '    "goal":"<研究目标，1句>",\n'
        '    "questions":["<子问题1>","<子问题2>",...],\n'
        '    "scopes":["<core scope id 列表>"],\n'
        '    "methods":["<方法>"],\n'
        '    "sources":["<来源>"],\n'
        '    "expected_outputs":["<输出>"]\n'
        "  }}\n"
        "}}\n"
        "仅输出 JSON，不要解释。FDA/法规/专利/供应链 等仅在确实相关时才纳入 core；普通消费品差评研究不应强制包含它们。"
    ).format(question=question, ctx=ctx_txt, catalog=scope_catalog, flags=json.dumps(flags, ensure_ascii=False))
    try:
        out = _call_llm_json(prompt, llm, "scope_suggestion")
        if isinstance(out, dict) and "scopes" in out:
            return out
    except Exception:
        pass
    return {"scopes": _suggest_scope_heuristic(question, context), "methods": [], "sources": [], "expected_outputs": [], "research_plan": {}}

# —— 统一 LLM JSON 调用（带重试 + 解析容错）——
def _call_llm_json(prompt, llm, task):
    import requests as _req
    sysmsg = "你是严谨的研究助理，只输出符合要求的 JSON，不要任何额外文字。"
    try:
        resp = _req.post(
            (llm.get("base") or "https://api.deepseek.com/v1") + "/chat/completions",
            headers={"Authorization": "Bearer " + llm["key"], "Content-Type": "application/json"},
            json={"model": llm.get("model") or "deepseek-chat", "messages": [
        ...
        raise RuntimeError("LLM 调用失败: {}".format(e))
    raw = content.strip()
    if raw.startswith("```"):
        raw = raw.strip("`")
...
# —— 动态 Research 报告生成（mode='dynamic'）——
def build_dynamic_report(question, context, plan, scopes, llm):
    ctx_lines = []
    for k in ("markets", "product_stage", "channels", "background", "product_type", "product_claims"):
        v = (context or {}).get(k)
        if v:
    ...
    ctx_txt = "\n".join(ctx_lines) or "（无额外背景）"
    plan = plan or {}
    scope_ids = plan.get("scopes") or [s.get("id") for s in (scopes or []) if s.get("level") == "core"]
    scope_titles = [RESEARCH_SCOPES.get(s, {}).get("title", s) for s in scope_ids]
    methods = plan.get("methods") or []
    sources = plan.get("sources") or []
    outputs = plan.get("expected_outputs") or []
    questions = plan.get("questions") or [question]
    goal = plan.get("goal") or question
    scope_block = "\n".join("- {}（{}）".format(RESEARCH_SCOPES.get(s, {}).get("title", s), RESEARCH_SCOPES.get(s, {}).get("desc", "")) for s in scope_ids)
    method_block = "\n".join("- {}".format(m) for m in methods) or "（由你按范围自行选择最合适的研究方法）"
    source_block = "\n".join("- {}".format(s) for s in sources) or "（由你按范围选择最权威来源）"
    output_block = "\n".join("- {}".format(o) for o in outputs) or "- 关键结论与产品建议"
    q_block = "\n".join("{}. {}".format(i+1, q) for i, q in enumerate(questions))
    prompt = (
        "你是资深产品研究员。请围绕下面的「研究问题」与「研究计划」产出一份结构化研究报告。\n\n"
        "【研究问题】\n{question}\n\n"
        "【背景 Context】\n{ctx}\n\n"
        "【研究目标】\n{goal}\n\n"
        "【研究范围 Scope（动态，非固定目录）】\n{scopes}\n\n"
        ...
        "要求：\n"
        "1. 报告章节必须围绕上述 Scope 与 Expected Outputs 动态组织，不要强行加入与本次研究无关的固定章节。\n"
        "2. 每个数字/结论标注来源；无法查证写「待核验」。\n"
        "3. 区分「已确认结论 / 证据不足 / 待验证假设」。\n"
        "4. 输出 JSON：\n"
        "{{\n"
        '  "topic":"{topic}",\n'
        '  "summary":"<总览，3-5句>",\n'
        ...
        '      "title":"<模块标题>",\n'
        '      "summary":"<该模块结论>",\n'
        '      "kpis":[...], "tables":[{{"title":"","headers":[...],"rows":[[...]]}}], "callouts":["<要点>"],\n'
        '      "note":""\n'
        "    }}\n"
        "  }},\n"
        '  "conclusions":["<关键结论1>","<关键结论2>"],\n'
        '  "recommendations":["<产品建议1>"]\n'
        "}}\n"
        "仅输出 JSON。"
    ).format(topic=question, question=question, ctx=ctx_txt, goal=goal, scopes=scope_block,
             qs=q_block, methods=method_block, sources=source_block, outputs=output_block)
    try:
        out = _call_llm_json(prompt, llm, "dynamic_report")
        if isinstance(out, dict) and "sections" in out:
            out["topic"] = out.get("topic") or question
            out["mode"] = "live"
            out["generatedAt"] = now_beijing()
            out["_dynamic"] = True
            return out
    except Exception:
        pass
    return {
        "topic": question, "mode": "live", "generatedAt": now_beijing(), "_dynamic": True,
        "summary": "（动态研究生成失败，请重试或检查后端 LLM 配置）",
        "kpis": [], "sections": {s: {"title": RESEARCH_SCOPES.get(s, {}).get("title", s), "summary": "生成失败", "kpis": [], "tables": [], "callouts": [], "note": ""} for s in scope_ids},
        "conclusions": [], "recommendations": [],
    }

# —— 历史 research_type → dimensions（运行时兼容，不物理迁移）——
RTYPE_TO_DIM_LEGACY = {
    "market": ["mkt"], "ecom": ["amz_us", "amz_eu"], "comp": ["comp"], "user": ["usr"],
    "tech": ["tech"], "reg": ["reg"], "patent": ["patent"], "sc": ["sc"],
    "prod": ["prod"], "clinical": ["clinical"], "other": ["other"], "comprehensive": ["mkt", "usr", "prod", "comp", "tech"],
}
def legacy_dimensions(rt):
    if not rt: return ["other"]
    return RTYPE_TO_DIM_LEGACY.get(rt, ["other"])
...
def _auto_archive(payload, report):
    ...
    try:
        ...
        rec = {
            ...
            # v2.3.0：动态 Research Creation Model 字段（运行时兼容，缺省走 legacy 映射）
            "research_question": (payload or {}).get("research_question"),
            "context": (payload or {}).get("context"),
            "scopes": (payload or {}).get("scopes"),
            "dimensions": (payload or {}).get("dimensions") or legacy_dimensions((payload or {}).get("research_type")),
            "primary_dimension": (payload or {}).get("primary_dimension"),
            "research_plan": (payload or {}).get("research_plan"),
            "recipe_id": (payload or {}).get("recipe_id"),
            "status": "completed",
...
def build_report(topic, dims, source="template", llm=None, mode="standard", question="",
                 research_question=None, context=None, scopes=None, methods=None, sources=None,
                 expected_outputs=None, research_plan=None, recipe_id=None, dimensions=None, primary_dimension=None):
    # v2.2.3：专题调研(topic 模式)下 topic 可能为空(前端只填了问题)，优先用 question 兜底，避免归档成「未命名调研主题」
    topic = (topic or "").strip() or (question or "").strip() or (research_question or "").strip() or "未命名调研主题"
    dims = [d for d in dims if d in DIMS]
    if not dims:
        dims = list(DIMS.keys())

    # —— v2.3.0：动态 Research Creation Model（mode='dynamic'）——
    # Research Question 决定 Research Scope；真正把 Question/Context/Scopes/Methods/Sources/
    # Expected Outputs/Research Plan 传入 AI，报告结构由 Research Plan 动态生成（非固定模板）。
    if mode == "dynamic":
        if not (llm and llm.get("key")):
            return {"error": "动态研究需要大模型实时生成（后端未配置 LLM Key）", "topic": topic, "mode": "dynamic", "sections": {}}
        return build_dynamic_report(research_question or topic, context or {}, research_plan or {}, scopes or [], llm)
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if _path in ("/", "/index.html"):
            ...
            self._send(200, {
                ...
                # v2.3.0：动态 Research Creation Model 能力目录（前端动态渲染 Scope/Recipe，不写死）
                "researchScopes": {k: {"title": v["title"], "icon": v["icon"], "desc": v["desc"], "methods": v.get("methods", []), "sources": v.get("sources", [])} for k, v in RESEARCH_SCOPES.items()},
                "researchRecipes": {k: {"title": v["title"], "default_scopes": v.get("default_scopes", []), "conditions": v.get("conditions", [])} for k, v in RESEARCH_RECIPES.items()},
    ...
    def do_POST(self):
        ...
        if _path == "/api/research":
            ...
            mode = payload.get("mode", "standard")   # v2.0: standard/topic/comprehensive/advanced/研究类型id；v2.3.0 新增 'dynamic'
            question = payload.get("question", "")   # v2.0: 专题调研问题
            # v2.3.0：动态 Research Creation Model 字段
            ...
            primary_dimension = payload.get("primary_dimension") or None
            ...
            # 仅保留已知维度；为空则默认全选（dynamic 模式不使用 dims，使用 scopes）
            dims = [d for d in dims if d in DIMS]
            if not dims:
                dims = list(DIMS.keys())
            if source == "llm" and not key:
                ...
                return
            result = build_report(
                topic, dims, source=source, llm=llm, mode=mode, question=question,
                research_question=research_question, context=context, scopes=scopes,
                methods=methods, sources=sources, expected_outputs=expected_outputs,
                research_plan=research_plan, recipe_id=recipe_id, dimensions=dimensions,
        ...
        elif _path == "/api/research/suggest-scope":
            # v2.3.0：动态 Research Scope 推荐（AI 判断 Research Scope）
            # 输入 {question, context{m markets/product_stage/channels/background/product_type/product_claims}}
            # 输出 {scopes[{id,name,level,reason}], methods, sources, expected_outputs, research_plan, recipe_id}
            rd_token = os.environ.get("RD_TOKEN")
            if rd_token:
                auth = self.headers.get("Authorization", "")
                q_token = ""
            ...
            try:
                ...
                self._send(400, {"error": "bad request: {}".format(e)})
                return
            sq = (payload.get("question") or "").strip()
            if not sq:
                self._send(400, {"error": "缺少 question 参数"})
                return
            sctx = payload.get("context") or {}
            # 优先用 LLM；未配置 Key 时回退启发式（仍可用）
            key = LLM_KEY
            llm_cfg = {"key": key, "base": LLM_BASE, "model": LLM_MODEL}
            if key:
                sug = _suggest_scope_llm(sq, sctx, llm_cfg)
            else:
                sug = {"scopes": _suggest_scope_heuristic(sq, sctx), "methods": [], "sources": [], "expected_outputs": [], "research_plan": {}}
            # 补全 research_plan（若 LLM 未给齐）
            plan = sug.get("research_plan") or {}
            plan.setdefault("goal", sq)
            plan.setdefault("scopes", [s.get("id") for s in sug.get("scopes", []) if s.get("level") == "core"])
            plan.setdefault("questions", [sq])
            plan.setdefault("methods", [m.get("name") for m in sug.get("methods", [])])
            plan.setdefault("sources", [s.get("name") for s in sug.get("sources", [])])
            plan.setdefault("expected_outputs", sug.get("expected_outputs") or [])
            sug["research_plan"] = plan
            sug["recipe_id"] = payload.get("recipe_id") or None
            self._send(200, sug)
        elif _path == "/api/reports":
