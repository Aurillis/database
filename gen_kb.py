...
.preview-safe{margin-left:10px;font-size:12px;color:#1a7f37;background:#e8f5ec;padding:2px 8px;border-radius:10px;font-weight:600;white-space:nowrap}
...
<div class="preview-overlay" id="preview">
  <div class="preview-header">
    ...
    <span id="previewSafe" class="preview-safe" style="display:none">🛡 沙箱隔离</span>
  ...
  <div class="preview-body">
    <iframe id="previewFrame" src="" sandbox="allow-scripts" referrerpolicy="no-referrer"></iframe>
  </div>
</div>
...
function openFile(filename, title) {
  ...
  // HTML：在沙箱 iframe 中渲染（不透明源，无法读取同域 localStorage / cookie，杜绝 XSS）
  var ext = fileExt(filename);
  if (ext === 'html' || ext === 'htm') {
    openHtmlSandboxed(filename, file, title);
    renderSidebar();
    return;
  }
...
function openHtmlSandboxed(filename, file, title) {
  var rawUrl = 'reports/' + encodeURIComponent(filename);
  var frame = document.getElementById('previewFrame');
  frame.removeAttribute('srcdoc');
  if (file.isUploaded && file.data) {
    // 会话内上传的 base64 文件：转 Blob 后加载（相对资源可能失效，仅展示用）
    try {
      var parts = file.data.split(',');
      var mime = (parts[0].match(/:(.*?);/) || [, 'text/html'])[1] || 'text/html';
      var bstr = atob(parts[1]);
      var n = bstr.length;
      var u8 = new Uint8Array(n);
      while (n--) u8[n] = bstr.charCodeAt(n);
    ...
    } catch(e) { window.open(rawUrl, '_blank'); return; }
  } else {
    // 已落盘文件：直接加载 reports/ 路径，相对图片/CSS 可正常显示；sandbox 使其运行在透明源
    frame.src = rawUrl;
  }
  document.getElementById('previewTitle').textContent = title || '报告预览';
  // 沙箱内文档不可信：关闭按钮改为「下载原文件」，避免新标签同域打开绕过沙箱
  var ob = document.getElementById('previewOpen');
  ob.href = rawUrl;
  ob.setAttribute('download', filename);
  ob.innerHTML = '<i class="fas fa-download"></i> 下载原文件';
  document.getElementById('previewSafe').style.display = '';
...
function closePreview() {
  ...
  document.getElementById('previewSafe').style.display = 'none';
  var ob = document.getElementById('previewOpen');
  if (ob) { ob.removeAttribute('download'); ob.innerHTML = '<i class="fas fa-external-link-alt"></i> 新标签打开'; }
}
