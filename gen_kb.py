...
.admin-entry{
  ...
  border:1px solid var(--line);border-radius:var(--radius);
  font-size:13px;font-weight:600;color:var(--ink);background:#fff;cursor:pointer;transition:all .15s;
}
...
.file-card-content{padding:14px;display:flex;flex-direction:column;gap:8px}
/* 封面 / 缩略图 */
.file-cover{
...
.file-cover.cover-img{background-size:cover;background-position:center;background-repeat:no-repeat}
.file-cover .cover-icon{font-size:44px;color:rgba(255,255,255,.95);filter:drop-shadow(0 1px 2px rgba(0,0,0,.18))}
.file-cover .cover-fmt{position:absolute;left:8px;bottom:8px;background:rgba(0,0,0,.55);color:#fff}
.file-cover .fav-btn{position:absolute;top:6px;right:6px;background:rgba(255,255,255,.85);box-shadow:0 1px 3px rgba(0,0,0,.15)}
.file-cover .fav-btn:hover{background:#fff}
.file-card-icon{
...
.fmt-badge{display:inline-flex;align-items:center;justify-content:center;font-size:10px;font-weight:700;letter-spacing:.3px;padding:2px 7px;border-radius:4px;line-height:1.4;white-space:nowrap}
.li-fmt{flex-shrink:0;display:flex;align-items:center;width:74px}
.fav-btn{
...
/* 搜索关键词高亮 */
mark.hl{background:#fff3a0;color:inherit;padding:0 1px;border-radius:2px;font-weight:600}
...
.list-item .li-icon.li-thumb{background-size:cover;background-position:center;background-repeat:no-repeat}
...
.admin-bar{position:sticky;top:56px;z-index:50;background:#fff;padding:8px 0 12px;margin-bottom:4px;border-bottom:1px solid var(--line)}
.admin-header{
  display:flex;align-items:center;justify-content:space-between;
  margin-bottom:16px;
}
.admin-tabs{display:flex;gap:2px;background:var(--hover);padding:3px;border-radius:8px}
.admin-tab{
...
# ===== HTML BODY =====
HTML = r"""
<!-- Font Awesome (本地托管，避免依赖国外 CDN 导致图标不显示) -->
...
function fileFmt(filename) {
  var ext = fileExt(filename);
  var labels = {
    'html':'HTML','htm':'HTML','css':'CSS','js':'JS',
    'pdf':'PDF',
    'png':'PNG','jpg':'JPG','jpeg':'JPG','gif':'GIF','svg':'SVG','webp':'WEBP','bmp':'BMP',
    'doc':'Word','docx':'Word',
...
function fmtColor(filename) {
  var ext = fileExt(filename);
  var map = {
    'html':'#e6783c','htm':'#e6783c','css':'#2965f1','js':'#c79f00',
    'pdf':'#c0392b',
    'png':'#16a34a','jpg':'#16a34a','jpeg':'#16a34a','gif':'#16a34a','svg':'#16a34a','webp':'#16a34a','bmp':'#16a34a',
    'doc':'#2563eb','docx':'#2563eb',
    'xls':'#1d7d3f','xlsx':'#1d7d3f','csv':'#1d7d3f',
    'ppt':'#d35400','pptx':'#d35400',
    'zip':'#6b7280','rar':'#6b7280','7z':'#6b7280','tar':'#6b7280','gz':'#6b7280',
    'mp4':'#7c3aed','mov':'#7c3aed','avi':'#7c3aed','mkv':'#7c3aed',
    'mp3':'#db2777','wav':'#db2777','flac':'#db2777',
    'txt':'#64748b','md':'#374151'
  };
  return map[ext] || '#888780';
}

// ===== 封面 / 缩略图 / 搜索高亮 辅助 =====
// 判断是否为可直接作缩略图的图片类型
function isImageFile(filename) {
  return ['png','jpg','jpeg','gif','webp','svg','bmp'].indexOf(fileExt(filename)) >= 0;
}

// 生成卡片封面区：图片文件用真实图片作封面；其余文件用「分类色渐变 + 大图标 + 格式徽标」生成的封面
function coverHtml(f, color, isFav) {
  var favBtn = '<button class="fav-btn'+(isFav?' active':'')+'" onclick="event.stopPropagation();toggleFav(\''+esc(f.filename)+'\')"><i class="fas fa-star"></i></button>';
  var fmt = '<span class="fmt-badge cover-fmt">'+fileFmt(f.filename)+'</span>';
  if (isImageFile(f.filename)) {
    var src = 'reports/' + encodeURIComponent(f.filename);
    return '<div class="file-cover cover-img" style="background-image:url(\''+src+'\')">'+fmt+favBtn+'</div>';
  }
  return '<div class="file-cover" style="background:linear-gradient(135deg,'+color+','+color+'cc)">'
    + '<i class="'+fileIcon(f.filename)+' cover-icon"></i>'+fmt+favBtn+'</div>';
}

// 搜索关键词高亮：先转义再对匹配词包裹 <mark class="hl">，避免 XSS
function hl(text) {
  ...
  try { return safe.replace(new RegExp('('+qEsc+')', 'gi'), '<mark class="hl">$1</mark>'); }
  catch (e) { return safe; }
}
...
function renderMain() {
  ...
  if (files.length === 0) {
    ...
    files.forEach(function(f) {
      ...
      html += coverHtml(f, color, isFav);
      html += '<div class="file-card-content">';
      html += '<div class="file-card-title">'+hl(f.title)+'</div>';
      html += '<div class="file-card-cat"><i class="'+catIcon(cat)+'"></i> '+hl(catName(cat))+'</div>';
    ...
    files.forEach(function(f) {
      ...
      if (isImageFile(f.filename)) {
        html += '<div class="li-icon li-thumb" style="background-image:url(\'reports/'+encodeURIComponent(f.filename)+'\')"></div>';
      } else {
        html += '<div class="li-icon" style="background:'+color+'15"><i class="'+fileIcon(f.filename)+'" style="color:'+color+'"></i></div>';
      }
      html += '<span class="li-title">'+hl(f.title)+'</span>';
      html += '<span class="li-cat">'+hl(catName(cat))+'</span>';
      html += '<span class="li-fmt"><span class="fmt-badge" style="background:'+fmtColor(f.filename)+'1a;color:'+fmtColor(f.filename)+'">'+fileFmt(f.filename)+'</span></span>';
...
function renderAdminFiles() {
  var c = document.getElementById('adminContent');
  var all = getAllFiles();
  var html = '<table class="admin-table"><thead><tr><th>文件名</th><th>分类</th><th>格式</th><th>大小</th><th>日期</th><th>操作</th></tr></thead><tbody>';
  ...
  all.forEach(function(f) {
    ...
    html += '<td><span class="fmt-badge" style="background:'+fmtColor(f.filename)+'1a;color:'+fmtColor(f.filename)+'">'+fileFmt(f.filename)+'</span></td>';
